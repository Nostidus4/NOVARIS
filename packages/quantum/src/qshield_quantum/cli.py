...
def _peak_memory_mb() -> float:
    ...
    `plan.md` E3 yêu cầu benchmark có "hard wall-time/memory/worker budget", nhưng
...
def _reference_hardware(config: Config) -> dict[str, object]:
    ...
    runtime là toàn bộ mục đích của benchmark (`plan.md` E3).
    """
    import platform
...
def _guard_exhaustive_budget(runtime_bits: int, config: Config) -> None:
    ...
    hỏng tệ nhất cho một pipeline có ngân sách (plan.md E3).
    ...
    if states > max_states:
        raise typer.BadParameter(
            ...
            "(plan.md E3: giá trị cần dry-run, không cam kết SLA)."
        )
