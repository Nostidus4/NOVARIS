...
def test_true_cvar_sign_and_cost_are_applied_once(risk_config: dict[str, Any]) -> None:
    ...
    assert result.cost_breakdown.total == pytest.approx(0.0002625)
    assert result.after.cvar["0.95"] == pytest.approx(0.0927625)
...
def test_g_pair_interaction_and_cost_handoff_by_hand(
    ...
    assert result.actions.loc[0, "c"] == pytest.approx(0.0000875)
