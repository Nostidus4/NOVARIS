...
## 2. Đối chiếu docs/decisions/2026-08-31-phan-hoi-bao-cao-15-08.md ↔ source code

### 2.1 Bảng trạng thái theo từng quyết định

| docs/decisions/2026-08-31-phan-hoi-bao-cao-15-08.md | Nội dung | Trạng thái | Bằng chứng (file:line) |
|---|---|---|---|
...
Coverage là thứ fail, stability thì luôn đạt. **N=15 là N nhỏ nhất qua được gate** — đúng con số docs/decisions/2026-08-31-phan-hoi-bao-cao-15-08.md D6/CR-WF2-005 dự đoán.
...
Đúng cảnh báo docs/decisions/2026-08-31-phan-hoi-bao-cao-15-08.md R09 và §4.5. Nghĩa là: **qua được candidate gate sẽ làm mất exact ground truth và làm QAOA statevector không chạy nổi trên laptop.** Không thể có cả hai cùng lúc với kiến trúc hiện tại.
...
Cùng instance sinh theo seed cố định, cùng settings (`shots=128, maxiter=10, reps=1, warm_start=False`), đo trên một máy. Đây là "thí nghiệm scaling có kiểm soát" mà docs/decisions/2026-08-31-phan-hoi-bao-cao-15-08.md R08 yêu cầu — khác hẳn việc so hai batch settings lệch nhau.
...
**Mức độ nghiêm trọng.** Mọi con số solver trong `workflow_benchmark.json` — `winning_bitstring`, `optimality_gap`, `success_prob`, `energy_stats`, `qaoa_beats_classical` — **không tái tạo được**, dù artifact mang đủ `registered_seeds`, `qubo_hash` và `candidate_order_hash`. Trực tiếp phá yêu cầu docs/decisions/2026-08-31-phan-hoi-bao-cao-15-08.md G3 (*"Ngọc ký final trên run/config hash cụ thể"*): ký vào một hash không dựng lại được con số.
...
Thí nghiệm này trả lời câu hỏi mở lớn nhất của review mà **không đổi một tham số tài chính nào** — nên nó hợp lệ theo docs/decisions/2026-08-31-phan-hoi-bao-cao-15-08.md §7 (*"chạy nghiên cứu có nhãn rõ"*) và không vi phạm A5 (*"gói nháp không kích hoạt runtime"*).
...
`sample_portfolio_weights` là **fixture thử nghiệm**, không phải tham số tài chính như `target_cash_increment` hay `financial_objective.*.weight`. Thay nó không phải "đổi số cho ra kết quả đẹp" mà là **sửa một thiết lập thử nghiệm không đại diện**: docs/decisions/2026-08-31-phan-hoi-bao-cao-15-08.md A1 định nghĩa persona là *"nhà đầu tư cá nhân có danh mục cổ phiếu hiện hữu"* — không ai nắm equal-weight cả 30 mã VN30.

Nhưng nó vẫn nằm trong `configs/` nên vẫn cần Ngọc duyệt, và **quan trọng hơn**: docs/decisions/2026-08-31-phan-hoi-bao-cao-15-08.md §7 ghi *"chưa có investor portfolio thật"*. Con số đúng phải đến từ một danh mục thật có consent (plan G2), không phải từ một dạng lệch do reviewer bịa ra.
...
- QAOA: 10 seed, 128 shots, maxiter 10, **warm-start TẮT** (theo `docs/decisions/2026-08-31-phan-hoi-bao-cao-15-08.md` E4: không prime bằng
...
`docs/decisions/2026-08-31-phan-hoi-bao-cao-15-08.md` C5 yêu cầu mã thiếu dữ liệu *"giữ residual risk hoặc block"*, không được biến mất. Test
